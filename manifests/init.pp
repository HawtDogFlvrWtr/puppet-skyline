...
class skyline (
  ...
  $alerts             = ['("skyline", "smtp", 1800),'],
  $hipchat_opts       = ['"rooms": { "skyline": (12345,),},','"color": "purple",'],
  $smtp_opts          = ['"sender": "skyline-alerts@etsy.com",','"recipients": {"skyline":["email@address.com", "email2@address.com"],},'],
  $pagerduty_opts     = ['"subdomain": "example",','"auth_token": "key",','"key": "key",',],
) {
    ...
    file { $analyzer_init:
        ...
        content => template('skyline/analyzer.init.erb'),
        owner   => 'root',
        group   => 'root',
        mode    => '0755',
        require => $require_target,
    ...
    file { $horizon_init:
        ...
        content => template('skyline/horizon.init.erb'),
        owner   => 'root',
        group   => 'root',
        mode    => '0755',
        require => $require_target,
    ...
    file { $webapp_init:
        ...
        content => template('skyline/webapp.init.erb'),
        owner   => 'root',
        group   => 'root',
        mode    => '0755',
        require => $require_target,
